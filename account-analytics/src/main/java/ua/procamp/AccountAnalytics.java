...
public class AccountAnalytics {
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(accounts -> accounts.getEmail().split("@")[1]));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(s -> s.getFirstName().length() + s.getLastName().length())
                .sum();
    }
    ...
    public BigDecimal calculateTotalBalance() {
        return accounts.stream().map(Account::getBalance).reduce(BigDecimal::add).orElse(new BigDecimal(0));
    }
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream().collect(toMap(Account::getId, accounts -> accounts));
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts
                .stream()
                .collect(Collectors.groupingBy(Account::getLastName, mapping(Account::getFirstName, toSet())));
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts
                ...
                .collect(Collectors.groupingBy(account -> account.getCreationDate().getMonth(),
                        mapping(Account::getBalance, Collectors.reducing(BigDecimal.ZERO, BigDecimal::add))));
    }
